...
parser.add_argument('--adv_file_path', default='../data/VOC2007/classification_mlliw_adv.csv', type=str,
                    metavar='N',
...
def evaluate_model(model):
    ...
    test_dataset = Voc2007Classification(args.data, 'val')
    test_dataset.transform = data_transforms
    test_loader = torch.utils.data.DataLoader(test_dataset,
                                              batch_size=args.batch_size,
                                              shuffle=False,
    ...
    metric = evaluate_metrics.evaluate(y, output, pred, model='millw')
    print(metric)
...
def evaluate_adv(state):
    ...
    adv_dataset = Voc2007Classification(args.data, 'mlliw_adv', inp_name='../data/VOC2007/voc_glove_word2vec.pkl')
    ...
    for i, max_r_limit in enumerate(max_r_limit_list):
        ...
        np.save('../attack_result/' + args.model_type + '/' + args.adv_method +
                '/mlliw_voc2007_{}_max_r_attack_fail_idx_{}.npy'.format(args.adv_method, max_r_limit),
                attack_fail_idx)
    ...
    for i, mean_r_limit in enumerate(mean_r_limit_list):
        ...
        if not os.path.exists('../attack_result/' + args.model_type + '/' + args.adv_method):
            os.makedirs('../attack_result/' + args.model_type + '/' + args.adv_method)
        np.save('../attack_result/' + args.model_type + '/' + args.adv_method +
                '/mlliw_voc2007_{}_mean_r_attack_fail_idx_{}.npy'.format(args.adv_method, mean_r_limit),
                attack_fail_idx)
...
def evaluate_adv_many(state):
    ...
    for i, add_of_mean in enumerate(add_of_means):
        ...
        if not os.path.exists('../attack_result/' + args.model_type + '/' + args.adv_method):
            os.makedirs('../attack_result/' + args.model_type + '/' + args.adv_method)
        np.save('../attack_result/' + args.model_type + '/' + args.adv_method +
                '/mlliw_voc2007_{}_mean_r_attack_fail_idx_{}.npy'.format(args.adv_method, add_of_mean),
                attack_fail_idx)
