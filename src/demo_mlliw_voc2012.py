...
parser.add_argument('--adv_method', default='BIM', type=str, metavar='N',
...
parser.add_argument('--adv_file_path', default='../data/VOC2012/classification_mlliw_adv.csv', type=str,
                    metavar='N',
...
def evaluate_adv(state):
    ...
    adv_dataset = Voc2012Classification(args.data, 'mlliw_adv', inp_name='../data/VOC2012/voc_glove_word2vec.pkl')
    ...
    for i, max_r_limit in enumerate(max_r_limit_list):
        ...
        np.save('../attack_result/' + args.model_type + '/' + args.adv_method +
                '/mlliw_voc2012_{}_max_r_attack_fail_idx_{}.npy'.format(args.adv_method, max_r_limit),
                attack_fail_idx)
    ...
    for i, mean_r_limit in enumerate(mean_r_limit_list):
        ...
        if not os.path.exists('../attack_result/' + args.model_type + '/' + args.adv_method):
            os.makedirs('../attack_result/' + args.model_type + '/' + args.adv_method)
        np.save('../attack_result/' + args.model_type + '/' + args.adv_method +
                '/mlliw_voc2012_{}_mean_r_attack_fail_idx_{}.npy'.format(args.adv_method, mean_r_limit),
                attack_fail_idx)
...
def evaluate_adv_many(state):
    ...
    for i, add_of_mean in enumerate(add_of_means):
        ...
        if not os.path.exists('../attack_result/' + args.model_type + '/' + args.adv_method):
            os.makedirs('../attack_result/' + args.model_type + '/' + args.adv_method)
        np.save('../attack_result/' + args.model_type + '/' + args.adv_method +
                '/mlliw_voc2012_{}_mean_r_attack_fail_idx_{}.npy'.format(args.adv_method, add_of_mean),
                attack_fail_idx)
...
if __name__ == '__main__':
    main()
    # mean_r_limit_list = [0.05, 0.5, 1]
    # metrics = []
    # for mean_r_limit in mean_r_limit_list:
    #     metrics.extend(np.load('../attack_result/mlgcn_voc2012_{}_mean_r_attack_fail_idx_{}.npy'.format('FGSM', mean_r_limit)))
