...
parser.add_argument('--adv_file_path', default='../data/NUSWIDE/classification_mlliw_adv.csv', type=str,
                    metavar='N',
...
def gen_adv_file(model, target_type, adv_file_path):
    ...
    y = y[0:200]
    y_target = y_target[0:200]
    new_image = new_image[0:200]
    new_tag = new_tag[0:200]
    ...
    with open(adv_file_path, 'w') as csvfile:
        ...
        for img, tag in zip(new_image, new_tag):
            ...
            row_info = ['images/' + img, str(tags), 'mlliw_adv']
...
def evaluate_model(model):
    ...
    test_dataset = NusWide(args.data, 'val')
    test_dataset.transform = data_transforms
    test_loader = torch.utils.data.DataLoader(test_dataset,
                                              batch_size=args.batch_size,
                                              shuffle=False,
...
def evaluate_adv(state):
    ...
    for i, max_r_limit in enumerate(max_r_limit_list):
        ...
        np.save('../attack_result/mlliw_nuswide_{}_max_r_attack_fail_idx_{}.npy'.format(args.adv_method, max_r_limit),
                attack_fail_idx)
    ...
    for i, mean_r_limit in enumerate(mean_r_limit_list):
        ...
        attack_fail_idx = np.argwhere(adv_pred_match_target == 0).flatten().tolist()
        np.save('../attack_result/mlliw_nuswide_{}_mean_r_attack_fail_idx_{}.npy'.format(args.adv_method, mean_r_limit),
                attack_fail_idx)
...
def evaluate_adv_show_many(state):
    model = state['model']
    y_target = state['y_target']
    y = state['y']
    y_target[y_target == -1] = 0
    ...
    max_r_limit_list = [1, 2, 3, 4]
    mean_r_limit_list = [mean_label, mean_label + 1, mean_label + 2, mean_label + 3]
    ...
    for i, max_r_limit in enumerate(max_r_limit_list):
        ...
        adv_pred_match_target = [(np.sum(adv_pred) > mean_label * max_r_limit) + 0]
        attack_fail_idx = np.argwhere(adv_pred_match_target == 0).flatten().tolist()
        np.save('../attack_result/mlliw_nuswide_{}_max_r_attack_fail_idx_{}.npy'.format(args.adv_method, max_r_limit),
                attack_fail_idx)
    ...
    for i, mean_r_limit in enumerate(mean_r_limit_list):
        ...
        adv_pred_match_target = [(np.sum(adv_pred) > mean_r_limit) + 0]
        attack_fail_idx = np.argwhere(adv_pred_match_target == 0).flatten().tolist()
        np.save('../attack_result/mlliw_nuswide_{}_mean_r_attack_fail_idx_{}.npy'.format(args.adv_method, mean_r_limit),
                attack_fail_idx)
...
def evaluate_adv_many(state):
    ...
    for i, add_of_mean in enumerate(add_of_means):
        ...
        attack_fail_idx = np.argwhere(adv_pred_match_target == 0).flatten().tolist()
        np.save('../attack_result/mlliw_nuswide_{}_mean_r_attack_fail_idx_{}.npy'.format(args.adv_method, add_of_mean),
                attack_fail_idx)
