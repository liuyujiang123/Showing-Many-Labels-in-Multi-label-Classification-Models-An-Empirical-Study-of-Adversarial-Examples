...
parser.add_argument('--model_path', default='../checkpoint/asl/COCO/MS_COCO_TRresNet_L_448_86.6.pth', type=str)
...
parser.add_argument('--adv_file_path', default='../data/COCO/asl_adv_anno.json', type=str, metavar='N',
                    help='all image names and their labels ready to attack')
parser.add_argument('--adv_save_x', default='../adv_save/asl/COCO/', type=str, metavar='N',
...
class NewModel(nn.Module):
    ...
    def forward(self, input):
        ...
        # catogry idx translate, because asl code are not consistent with mlgcn code
        o = o[[4, 47, 24, 46, 34, 35, 21, 59, 13, 1,
               14, 8, 73, 39, 45, 50, 5, 55, 2, 51,
               15, 67, 56, 74, 57, 19, 41, 60, 16, 54,
               20, 10, 42, 29, 23, 78, 26, 17, 52, 66,
               33, 43, 63, 68, 3, 64, 49, 69, 12, 0,
               53, 58, 72, 65, 48, 76, 18, 71, 36, 30,
               31, 44, 32, 11, 28, 37, 77, 38, 27, 70,
               61, 79, 9, 6, 7, 62, 25, 75, 40, 22]]
        o = torch.transpose(o, 1, 0)
        return o
...
def gen_adv_file(model, target_type, adv_file_path):
    ...
    test_dataset = COCO2014(args.data, phase='val')
    ...
    for file in adv_labeled_data:
        ...
        img.save('../data/COCO/asl_adv/{}'.format(filename))
    write_object_labels_json('../data/COCO/asl_adv_anno.json', adv_labeled_data)
    np.save('../adv_save/asl/COCO/y_target.npy', y_target)
    np.save('../adv_save/asl/COCO/y.npy', y)
...
def evaluate_adv(state):
    ...
    adv_dataset = COCO2014(args.data, phase='asl_adv')
    ...
    for i, max_r_limit in enumerate(max_r_limit_list):
        ...
        np.save('../attack_result/asl_coco_{}_max_r_attack_fail_idx_{}.npy'.format(args.adv_method, max_r_limit),
                attack_fail_idx)
        ...
        write_to_excel(metrics, base_row=36, args=args, sheet=i, filename='max_r_result.xls')
    ...
    for i, mean_r_limit in enumerate(mean_r_limit_list):
        ...
        attack_fail_idx = np.argwhere(adv_pred_match_target == 0).flatten().tolist()
        np.save('../attack_result/asl_coco_{}_mean_r_attack_fail_idx_{}.npy'.format(args.adv_method, mean_r_limit),
                attack_fail_idx)
        ...
        print()
        logging.info(str(metrics))
        write_to_excel(metrics, base_row=36, args=args, sheet=i, filename='mean_r_result.xls')
...
def main():
    ...
    # value is {-1,1}
    y_target = np.load('../adv_save/asl/COCO/y_target.npy')
    y = np.load('../adv_save/asl/COCO/y.npy')
