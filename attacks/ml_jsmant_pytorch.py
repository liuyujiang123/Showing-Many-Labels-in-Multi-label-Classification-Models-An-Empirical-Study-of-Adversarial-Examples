...
class MLJSMANT(object):
    ...
    def generate_np(self, x, **kwargs):
        ...
        logging.debug("start jsmant attacking!")
        ...
        while np.any(current != y_target) and iteration < self.gamma:
            ...
            for idx in range(sample.shape[0]):
                ...
                while disturbe_num < 2500:
                    ...
                    disturbe = compute_disturbe(idx,feature_max_idx1,feature_max_idx2,feature_max_idx3,x_shape,sample_old)
                    sample[idx][feature_max_idx1][feature_max_idx2][feature_max_idx3] = disturbe
...
def compute_disturbe(idx,idx0,idx1,idx2,x_shape,sample_old):
    if idx1 > 0 and idx1 < x_shape[1]-1 and idx2 > 0 and idx2 < x_shape[2]-1:
        disturbe = (sample_old[idx][idx0][idx1-1][idx2-1] + sample_old[idx][idx0][idx1+1][idx2+1] + sample_old[idx][idx0][idx1-1][idx2+1] + sample_old[idx][idx0][idx1+1][idx2-1])/4
    else:
        disturbe = sample_old[idx][idx0][idx1][idx2] + 0.015
    return disturbe
