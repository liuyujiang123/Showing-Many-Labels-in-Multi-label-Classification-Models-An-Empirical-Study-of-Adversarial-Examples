        ...
        while iteration < max_iter:
            ...
            for i in range(num_instaces):
                if (best_pred[i] == y_target[i]).all() and (i not in error_idx):
                    ...
                    print("No new adversarial example found")
            ...
            all_time = []
            for i in range(num_instaces):
                item = [np.expand_dims(jac_grad[i], 0), np.expand_dims(output[i], 0), np.expand_dims(y_target[i], 0),
                        threshold_value[i], np.expand_dims(r_change[i], 0)]
            ...
            #     begin_time = time.time()
            #     arg = (
            #         np.expand_dims(jac_grad[i], 0),
            #         np.expand_dims(output[i], 0),
            #         np.expand_dims(y_target[i], 0),
            #         threshold_value[i],
            #         np.expand_dims(r_change[i], 0)
            #     )
            #
            #     # 直接调用函数，并将结果存储在results列表中
            #     re = mosek_inner_point_solver(*arg)
            #     result.append(re)
            #     end_time = time.time()
